...
**N人の人間 × N体のぷち**構成に対応しています(v0.3〜)。v0.4ではキャラクター単位の会話ロックとグループ会話が加わりました。`PETIT_DATA_DIR/characters/` 配下にキャラクターのディレクトリを1つ作るだけでキャラクターが増え(再起動不要。M5デバイス連携のみ起動時に固定)、`users.json` に人間のアカウントを増やすだけで家族それぞれのログインが増えます。ありさんの家(2人×3ぷち)のような構成がそのまま動く一般形です。
...
- 💬 **会話** — Web UIから直接Claude(キャラクター)とテキストチャット。M5のマイク/カメラ/センサー起動時の自動応答もここに記録される。同じキャラクターへの会話は「1キャラクター=1つの心」の原則でキャラクター単位のロックにより直列化され、他の人と話し中は「◯◯と話し中…」と表示される(下記「会話の排他制御」参照)
- 👨‍👩‍👧‍👦 **グループ会話**(v0.4〜) — ログインユーザーに見えていて `in_group: true`(デフォルト)のキャラクター全員に一度に話しかけられる。返事は届いた子から順にストリーミング表示され、直前の子の返答は次の子への文脈として渡される。ログはユーザーごとに別々(家族で共有はしない)
...
| `in_group` | グループ会話に参加するか(v0.4〜) | `true` |

## 会話の排他制御(1キャラクター=1つの心)

同じキャラクターへの会話リクエスト(1対1チャット・グループ会話・M5ボタンの自動応答・日記生成、すべて)は、キャラクターIDごとのロックで直列化されます — 1体のぷちが同時に2つの会話をしないようにするためです。別のキャラクターへのリクエストは完全並行に処理されます。

- 処理中に別の人からリクエストが来た場合はロックの解放待ちになり、Web UIは `GET /api/<character_id>/chat/status` をポーリングして「◯◯(話し中の相手の表示名)と話し中…」と表示します(自分自身の別リクエストは「話し中」表示の対象外)。
- ロックは120秒(`CHAR_LOCK_TIMEOUT`)待っても解放されない場合、強制的に開放されます(claude CLI呼び出し自体もこの秒数でタイムアウトするので、通常は二重の安全弁として働きます)。
...
├── users/<user_id>/group_chat.json        # ★グループ会話ログ(v0.4〜: ユーザーごとに別々、他ユーザーには見えない)
